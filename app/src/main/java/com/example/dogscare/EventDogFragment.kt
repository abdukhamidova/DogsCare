...
import com.example.dogscare.databinding.FragmentEventDogBinding
...
class EventDogFragment : Fragment() {
    private lateinit var binding: FragmentEventDogBinding
    val database = FirebaseFirestore.getInstance()
    val user = FirebaseAuth.getInstance().currentUser
    private var originalEvent: DogEvent? = null
    ...
    override fun onCreateView(
        ...
        binding = FragmentEventDogBinding.inflate(inflater, container, false)
        val fireDogId = arguments?.getString("fireDogId")
        ...
        if(fireDogId != null) {
            if (activity is ActivityAdd) {
                ...
                buttonSave.setOnClickListener {
                    addEventToDatabase(fireDogId)
                }
            }
            else if (activity is ActivityEvent) {
                (activity as? ActivityEvent)?.setToolbarTitle("Wydarzenie psa")
                val fireEventId = arguments?.getString("fireEventId")
                val isArchive = arguments?.getBoolean("isArchive")
                ...
                if (fireEventId != null) {
                    fetchEventData(fireDogId, fireEventId)

                    buttonSave.setOnClickListener{
                        updateEventData(fireDogId, fireEventId)
                    }
                    ...
                    buttonDelete.setOnClickListener{
                        if(user != null){
                            val ref = database.collection("users").document(user.uid)
                                ...
                                .collection("events").document(fireEventId)
    ...
    private fun addEventToDatabase(fireDogId: String){
        if (user == null) {
        ...
        val itsInjection = binding.radioButtonInjection.isChecked
    ...
    private fun fetchEventData(fireDogId : String, fireEventId : String){
        if (user == null) {
            Toast.makeText(requireContext(), "Błąd: użytkownik niezalogowany!", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        database.collection("users").document(userId)
            ...
            .get().addOnSuccessListener { document ->
                if (document != null && document.exists()) {
                    val dogEvent = document.toObject(DogEvent::class.java)
                    ...
                    dogEvent?.let { event ->
                        ...
                        if(event.injection) binding.radioButtonInjection.isChecked = true
    ...
    private fun updateEventData(fireDogId: String, fireEventId : String){
        if (user == null) {
            Toast.makeText(requireContext(), "Błąd: użytkownik niezalogowany!", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        originalEvent?.let{original ->
            ...
            if(itsInjection != original.injection) updates["injection"] = itsInjection
        }
